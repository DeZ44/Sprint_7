package data;

public class CourierCredentials {
    private String login;
    private String password;

    public CourierCredentials(String login, String password) {
        this.login = login;
        this.password = password;
    }

    public CourierCredentials(String login) {
        this.login = login;
    }

    public static CourierCredentials from(CourierData courier){
        return new CourierCredentials(courier.getLogin(), courier.getPassword());
    }

    public static CourierCredentials of(CourierData courier){
        return new CourierCredentials(courier.getLogin());
    }

    public static CourierCredentials noLogin(CourierData courier){
        return new CourierCredentials(courier.getPassword());
    }

    public static CourierCredentials badPassword(CourierData courier){
        return new CourierCredentials(courier.getLogin(), "1");
    }


    public CourierCredentials() {
    }

    public String getLogin() {
        return login;
    }

    public void setLogin(String login) {
        this.login = login;
    }

    public String getPassword() {
        return password;
    }

    public void setPassword(String password) {
        this.password = password;
    }
}
